...
function notifyConfig(trigger, file) {
  this.rules.forEach(function(rule) { rule.notify(trigger, file); });
}
function notifyRule(cfg, trigger, file) {
  //console.log('Rule: '+this.name);
  this.source.forEach(function(item) { item.notify(trigger, file); });
}
function notifySource(cfg, rule, trigger, file) {
  //console.log('Source('+(typeof file)+'): '+this.source);
  var that = this;
  var siblings = rule.source.map(function(source) { return source.source; });
  [ that.source ].concat(that.dependencies).filter(function(item) {
    return ('undefined' === typeof file) || (item === file);
...
function checkAllDepend(callback) {
  async.forEach(this.source || this.rules, function(item,cb) { item.checkDepend(cb) }, callback);
}
