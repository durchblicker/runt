...
function start() {
  ...
  nextTick(function() {
    async.whilst(function() { if (!tasks.length) tasks=undefined; return tasks?true:false; }, function(callback) {
...
function execute(task, callback) {
  fs.stat(task.file, function(err, stat) {
    ...
    task.stat = stat;
    setTimeout(function() { schedule(task); }, task.interval);
    callback();
  });
}
...
function watch(files, interval) {
  ...
  files.forEach(function(file) {
    schedule({ file:file, emitter:emitter, interval:interval||1000 });
  });
  return emitter;
}
