...
function watch(config, log) {
  ...
  Pea.map(config.rules, findsources.bind(log)).success(function() {
    Pea.map(config.rules, dependencies.bind(log)).success(function() {
      config.rules.forEach(function(rule) {
        rule.resolved = rule.resolved.map(function(path) { return Path.resolve(config.base, path); });
        rule.dependent = rule.dependent.map(function(path) { return Path.resolve(config.base, path); });
      });
      fsevents = fsevents(config.base);
      fsevents.on('change', function(path, info) {
        config.rules.forEach(function(rule) {
...
function findsources(rule, callback) {
  ...
  if ('string' === typeof rule.source) {
    ...
    rule.resolved = [].concat(Array.isArray(rule.source) ? rule.source : [ rule.source ]);
    callback();
  }
}
...
function buildRule(log, module, source, sources, target, options, callback) {
  if ('string' === typeof target) {
    ...
    module.individual(source, dest, options, done(dest));
  } else {
